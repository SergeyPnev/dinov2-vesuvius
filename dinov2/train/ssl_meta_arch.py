...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        print("cfg: ", cfg.student.pretrained_weights)
        if cfg.student.pretrained_weights:
            chkpt = torch.load(cfg.student.pretrained_weights)
            logger.info(f"OPTIONS -- pretrained weights: loading from {cfg.student.pretrained_weights}")
            student_backbone.load_state_dict(chkpt["model"], strict=False)

        student_backbone.patch_embed = PatchEmbed(img_size=224, patch_size=14, in_chans=16, embed_dim=768)
        teacher_backbone.patch_embed = PatchEmbed(img_size=224, patch_size=14, in_chans=16, embed_dim=768)
        student_backbone.pos_embed = nn.Parameter(torch.zeros(1, 257, embed_dim))
        teacher_backbone.pos_embed = nn.Parameter(torch.zeros(1, 257, embed_dim))
