...
class DataAugmentationDINOVC(object):
    def __init__(
            self,
            global_crops_scale,
            local_crops_scale,
        ...
        # random resized crop and flip
    # Geometric augmentations (Global)
        self.geometric_augmentation_global = A.Compose(
            [
                A.RandomResizedCrop(
                    global_crops_size, global_crops_size, scale=global_crops_scale, interpolation=cv2.INTER_CUBIC
                ),
                A.HorizontalFlip(p=0.5),
            ]
        )

        # Geometric augmentations (Local)
        self.geometric_augmentation_local = A.Compose(
            [
                A.RandomResizedCrop(
                    local_crops_size, local_crops_size, scale=local_crops_scale, interpolation=cv2.INTER_CUBIC
                ),
                A.HorizontalFlip(p=0.5),
            ]
        )
        ...
        # Global transformation 1 extra
        global_transfo1_extra = A.GaussianBlur(blur_limit=(3, 7), p=1.0)

        # Global transformation 2 extra
        ...
        # Local transformation extra
        local_transfo_extra = A.Compose(
            [
        ...
        self.normalize = A.Compose(
            [
                A.Normalize(mean=[0]*num_channels, std=[1]*num_channels),
                ToTensorV2(),
            ]
        )
    ...
    def __call__(self, image):
        ...
        # local crops:
        local_crops = [
            self.local_transfo(image=self.geometric_augmentation_local(image=image)["image"])["image"] for _ in range(self.local_crops_number)
        ]
        output["local_crops"] = local_crops
        output["offsets"] = ()
