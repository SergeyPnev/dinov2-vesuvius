...
class CFG:
    ...
    comp_dir_path = "./"
    comp_folder_name = "./"
#     comp_dataset_path = f"/raid/cian/user/sergei.pnev/data/VC_HRDA/scroll_4/"
...
def read_image_mask(fragment_id, start_idx=17, end_idx=43, CFG=CFG, fold="train_scrolls"):
    ...
    if fragment_id in ['20231106155351',
        ...
        mask = cv2.imread(
            CFG.comp_dataset_path
                + f"{fold}/{fragment_id}/{fragment_id}_inklabels_refined.png",
            0,
            )
    else:
        if any(id_ in fragment_id for id_ in ["20231022170901", "20231022170900"]):
            mask = cv2.imread(
                CFG.comp_dataset_path
                + f"{fold}/{fragment_id}/{fragment_id}_inklabels.tiff", 0
            )
        else:
            mask = cv2.imread(
                CFG.comp_dataset_path
                + f"{fold}/{fragment_id}/{fragment_id}_inklabels.png", 0
            )
    if fold == "train_scroll_235":
...
class TrainDataset(Dataset):
    def __init__(self, df_path, cfg, transform=None):
        self.df = pd.read_csv(df_path)
        self.paths = self.df["images"].values
        self.cfg = cfg
    ...
    def __getitem__(self, idx):
        ...
        image = np.zeros((self.size, self.size, self.in_channels)).astype(np.float32)

        the_rawest_path = raw_path.split("/")[-1]
#         for i in range(16):
#             image[..., i] = cv2.imread(f"{self.root}{raw_path}_{i}.jpg", 0).astype(np.float32)
        image = tiff.imread(f"{self.root}img/{raw_path}.tif").astype(np.float32)
        mask = cv2.imread(f"{self.root}gt/{raw_path}.png", 0)
...
class RegressionPLModel(pl.LightningModule):
    ...
    def forward_backbone(self, x, B, out_indices=[1, 2, 3]):
        with torch.no_grad():
            x = backbone.forward_features(x, out_indices=out_indices)["x_norm_patchtokens"]
            x = x.permute(0, 2, 1)
...
#         print("train: ", outputs.shape, y.shape)
    ...
    def validation_step(self, batch, batch_idx):
        x, y, xyxys = batch
#         print("val data: ", x.shape, y.shape)
        batch_size = x.size(0)
        outputs = self.forward(x, out_indices=[])
#         outputs = self.forward(x)
#         print("val: ", outputs.shape, y.shape)
        loss1 = self.loss_func(outputs, y)
...
#         "/raid/cian/user/sergei.pnev/VC/VC_224/scroll_1.csv",
#         CFG,
...
#     )
    ...
    torch.save(model.state_dict(), CFG.outputs_path + "/baseline_190924_model_0093749_s1s5.pth")
    wandb.finish()
